...
| Malicious site correlating users                             | `sub` is a hash of a per-site key; `idz` is never released to sites. The handle is released only to sites that request the `handle` scope, and only if the user set one.                                                                                                                                    |
...
- **Routing metadata.** The index necessarily learns which sites an identity signs in to. Sites cannot correlate a person across each other; an organisation that must keep this metadata private runs its own index.
