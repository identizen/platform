...
| Device key          | Fresh Ed25519 keypair per install; **not derived**                                                | Identifies the _install_, not the person. Signs the `device_sig` on assertions and `Idz-Signature` request headers (registration itself is proved by the master key, §8.1). Stored in the platform keychain / keystore, biometric-gated when enabled; Secure Enclave / StrongBox wrapping is planned. |
...
| `amr`                            | What verified the person at the gate, in RFC 8176 terms: `face`, `fingerprint`, `iris`, `pin` (device passcode), `user` (a biometric did, the platform cannot say which), `swk` (nothing did; a software-held key signed, development only). A phone reports what actually happened and never more: a passcode fallback is `pin`, a skipped prompt is `swk`. `hwk` is reserved for hardware-isolated keys and is not asserted by any Identizen client today. The index refuses an `idz:mfa` assertion without a user-verifying value (`403 insufficient_amr`). |
...
1. Load the challenge session by `challenge_id`; reject if unknown (`404 unknown_challenge`), already resolved or expired (`410 challenge_<status>`).
2. Look up `payload.device_id`; reject if unknown (`401 unknown_device`) or `status ≠ active` (`403 device_inactive`), so every later failure can be attributed to a device.
3. Check who may answer. A challenge issued for a person (step-up `login_hint`, Verification API `sub`) records that identity at creation: the device must belong to it (`403 wrong_identity`). An untargeted challenge that discovery routed to one device is that device's (`403 wrong_device`).
