...
test('paired browsers: list and unpair', async ({ page }) => {
  ...
  await expect(list.getByText('Safari 17 on macOS')).toBeVisible();
  await page.getByRole('button', { name: 'Unpair Safari 17 on macOS' }).click();
  ...
  await expect(page.getByRole('button', { name: 'Unpair Safari 17 on macOS' })).toHaveCount(0);
});
