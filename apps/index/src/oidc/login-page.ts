...
(function(){
  ...
  if(!cfg.pushed){ensureBrowserKey().then(function(k){return crypto.subtle.exportKey('raw',k.publicKey);}).then(function(raw){return fetch(cfg.indexUrl+'/challenge/'+cfg.challengeId+'/browser-key',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({browser_pubkey:b64(raw)})});}).catch(function(){}).then(tryPaired).then(function(){connect();});}else{connect();}
