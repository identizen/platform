/**
 * Local copy of packages/protocol/src/vectors.ts (that file is not exported from the package).
 * It only calls the public @identizen/protocol API, so running it on Hermes proves the protocol
 * package produces byte-identical output on the mobile JS engine. Keep in sync with the original.
 */
import {
  canonicalize,
  createAssertion,
...
export function generateVectors(): Record<string, unknown> {
  ...
  const keys = {
    ...
    index: { private_hex: VECTOR_INDEX_KEY_HEX, public_b64url: toBase64Url(index.publicKey) },
    identity_proof: {
  ...
  const challenge = {
    login: {
      ...
      signing_input: decode(signingBytes('challenge', loginSigned.payload)),
      sig: loginSigned.sig,
    },
    mfa: {
      challenge: mfaSigned.payload,
      signing_input: decode(signingBytes('challenge', mfaSigned.payload)),
      sig: mfaSigned.sig,
    },
  };
  const assertion = {
    login: {
      ...
      signing_input: decode(signingBytes('assertion', loginAssertionSigned.payload)),
    ...
    mfa: {
      ...
      signing_input: decode(signingBytes('assertion', mfaAssertionSigned.payload)),
  ...
  const pairing = {
    ...
    signing_input: decode(signingBytes('pairing', pairingSigned.payload)),
