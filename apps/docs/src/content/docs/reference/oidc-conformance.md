...
| Discovery §3                 | response_types_supported is honoured and unadvertised response types are rejected                          |
| Discovery §3                 | grant_types_supported is honoured and unadvertised grants are rejected with unsupported_grant_type         |
| Discovery §3                 | code_challenge_methods_supported is honoured (S256) and plain or missing methods are rejected              |
...
| Discovery §3 / RFC 6749 §3.3 | every advertised scope is honoured; unadvertised scopes are ignored and absent from the granted scope      |
...
| CORS                         | /token and /userinfo reflect the request Origin (index behaviour, not a spec requirement)                  |
...
| Core §3.1.2.1                 | an unknown prompt value is carried through without error (index behaviour; the spec defines no error for it)                                                    |
...
| Core §3.1.2.1                 | login_hint with a bound sub is honoured; an unbound sub yields login_required (Identizen reads login_hint as the per-site sub)                                  |
| Core §2 / §3.1.2.1            | auth_time is present in the id_token when max_age was requested — **skipped**, see [deviations](#deliberate-deviations)                                         |
...
| Back-Channel §2 / Discovery                   | backchannel_logout_supported and backchannel_logout_session_supported are advertised and honoured                                 |
...
| `auth_time` is not issued, even with `max_age` (Core §2, §3.1.2.1 says it is REQUIRED then).                                                                                  | Every login is a fresh phone approval, so `auth_time` would equal `iat`. Adding an id_token claim is a protocol decision that has not been taken; the assertion is skipped in the suite and listed here so it is not forgotten.                                                                                                                                                      |
...
| CORS reflects any `Origin` on every route, including `/token` and `/userinfo`.                                                                                                | Public (PKCE-only) clients in the browser call `/token` directly. This is index behaviour, not a specification requirement; the suite documents it rather than requiring it.                                                                                                                                                                                                         |
...
- **Review** — `oidcc-prompt-login` (a person confirms the second login page was shown), `oidcc-ensure-registered-redirect-uri` and `oidcc-ensure-request-object-with-redirect-uri` (a person confirms the index showed an error instead of redirecting to the unregistered URI, which it does: a JSON `400 invalid_request`). Review is the highest result those modules give.
...
- **Failed, by design** — `oidcc-prompt-none-logged-in` and `oidcc-id-token-hint` expect a silent login for a person the index already knows; every Identizen login is a phone approval, so `prompt=none` is always `interaction_required` ([deviations](#deliberate-deviations)). `oidcc-login-hint` sends an arbitrary hint; the index accepts only a bound `sub` and answers `login_required`.
- **Failed, fixable** — `oidcc-max-age-1` and `oidcc-max-age-10000`: the id_token has no `auth_time`, which Core §2 requires when `max_age` was requested. Issuing it (the time of the phone approval) is the one change that would turn both green; it is an id_token claim change and is tracked as an open decision rather than made quietly.

### Results

Run on 2026-09-06 against the index at the commit that added this page, with the procedure above (`OIDC_PKCE_OPTIONAL=true`, confidential client, `client_secret_basic`). Suite plan `Y95zKwO5Rxdam`; `oidcc-userinfo-get` and `oidcc-codereuse-30seconds` were rerun alone (plans `EjTp9DVWlksW1`, `U9b1hGNyTzkX8`) after a stray screenshot upload had marked them for review.
...
| Review  |     3 | `oidcc-prompt-login`, `oidcc-ensure-registered-redirect-uri`, `oidcc-ensure-request-object-with-redirect-uri`                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Failed  |     5 | `oidcc-prompt-none-logged-in`, `oidcc-id-token-hint`, `oidcc-login-hint` (by design); `oidcc-max-age-1`, `oidcc-max-age-10000` (no `auth_time`)                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
Update this table when you rerun the plan, and keep the failed rows honest: a certification would need the two design deviations resolved or accepted, and `auth_time` issued.
