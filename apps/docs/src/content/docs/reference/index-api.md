...
The one unsigned request. Creates the identity on first sight of a master key. A device key is enrolled at most once: an active key registering again gets its existing enrollment back (`200`), a revoked key is refused (`403 device_revoked`).
