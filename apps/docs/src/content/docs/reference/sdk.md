...
- `register-site` prints the credentials once as JSON. `--live` issues an `idz_live_…` id; `--public` creates a PKCE-only client with no secret. `--token` carries the registration token for an index with closed registration, which includes the hosted one (see [`registration_closed`](/errors/#registration_closed)).
