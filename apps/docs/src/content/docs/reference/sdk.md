...
Creates a challenge and runs discovery: **paired browser** (push straight to the phone) → **Web Bluetooth** (Chromium, behind feature detection) → **QR**; on a mobile user agent the deep link is used instead. Returns a `LoginSession`:
...
- `<IdentizenProvider indexUrl clientId pairing? client?>` — one per app; `client` injects a pre-built client (tests).
...
- `useIdentizen()` → `{ client, state, busy, startLogin, enroll, stepUp, cancel, reset }`.

Style through your own classes or the `data-idz` attributes (`button`, `panel`, `status`, `code`, `qr`, `deeplink`, `cancel`, `retry`, `step-up`, `reason`, `start`).

## `identizen` CLI

```bash
npx identizen init [--index <url>] [--name <site name>] [--site-url <url>] [--framework next|express] [--force] [--token <t>]
npx identizen dev [--index <url>] [--port 4400] [--policy approve|deny|manual|ignore] [--local]
npx identizen register-site --name <n> --rp-id <host> --redirect-uri <uri> [--redirect-uri <uri>…] \
