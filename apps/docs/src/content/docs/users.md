...
- **Several indexes.** One phone can hold the same identity on the public index and on an organization's index at once (app 0.2.0 or later); `sub` is the same everywhere because it never depends on the index. Restoring from the 24 words brings back the personal identity on the public index; organization indexes are added again by enrolling again.
