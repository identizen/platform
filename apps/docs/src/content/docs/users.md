...
| Not correlatable           | Two sites comparing their `sub` values, or their `idz_device` values, learn nothing: both are derived per site.                                                              |
...
- **Several indexes.** One phone can hold the same identity on the public index and on an organization's index at once (from the next app build); `sub` is the same everywhere because it never depends on the index. Restoring from the 24 words brings back the personal identity on the public index; organization indexes are added again by enrolling again.
