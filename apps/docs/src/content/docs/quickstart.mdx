...
You need Node 22+ and an index to talk to. For local development run the index from the monorepo (`npm run dev -w @identizen/index`, listens on `http://localhost:8787`) or point at a hosted index with `--index`. Registration on the hosted index is by token: pass `--token` to `identizen init` with the token you were given (ask at [contact@identizen.com](mailto:contact@identizen.com)), or use a local index, which registers anything.
