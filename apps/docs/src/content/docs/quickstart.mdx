...
You need Node 22+ and an index to talk to. For local development run the index from the monorepo (`npm run dev -w @identizen/index`, listens on `http://localhost:8787`) or point at a hosted index with `--index`: ours at `https://index.identizen.com`, or [your own](/self-hosting/) on your Cloudflare account. It is the same code either way, and your users use the same Identizen app. On the hosted index, registration is open and a live site starts working once it proves its domain: `identizen init` prints a DNS TXT record (or a file to serve at `/.well-known/identizen-site`); publish it and run `npx identizen verify-site --client-id …`. A local index verifies nothing, and `localhost` never needs it.
...
4. **Log in**
   ...
   The iPhone app is on the [App Store](https://apps.apple.com/app/identizen/id6808322640) and the Android app is coming to Google Play. Without a phone at hand, use the fake phone from `identizen dev`, or the playground at https://identizen.com/playground. The [examples](/examples/) page lists sample apps you can run the same way.
