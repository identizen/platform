...
| OIDC surface           | Authorization Code with PKCE S256 only, no refresh tokens; the id_token claims in section 5 (`iss, sub, aud, iat, exp, nonce, sid, amr, acr, at_hash, idz_device`, optional `idz_handle` and `idz_org`); no email claim |
...
- **`hwk` in `amr`.** The value anticipates hardware-isolated key storage that has not shipped. Treat it as advisory until the threat model says otherwise. The array shape does not change; the meaning of `hwk` tightens.
