...
`index` is the tenant issuer, so the phone knows which index to talk to before anything else happens. The link is shown once, works for one phone, and stops working when it expires or when a helpdesk user or administrator revokes it from **Approvals** (status filter `pending`, then **Revoke**, which audits `enrollment.revoked`). Neither the portal nor the org app sends the link anywhere: the administrator copies it and passes it on, or the member scans the code from the org app's own screen. Email delivery is on the [roadmap](/enterprise/#on-the-roadmap).
...
**Policy** in the portal (owners and admins edit; auditors read; the index also lets helpdesk read it over the API) holds four settings. Each one is applied when a phone claims, using the snapshot taken when the token was issued. Saving audits `policy.updated` and an admin action `policy.update` with the old and new values.
...
Nothing else is policy yet: there is no geo or time rule, no biometric class, no session maximum age and no `allowed_amr`; those stay on the [roadmap](/enterprise/#on-the-roadmap), as do scheduled re-checks of enrolled devices.
