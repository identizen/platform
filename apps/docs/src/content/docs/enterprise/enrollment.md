...
| ---------------------- | :-----: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `require_attestation`  |   off   | The claim must carry an attestation that the index verified (`verified`). A claim without one is refused with `attestation_required`; one the index cannot verify because it lacks credentials for that platform is refused too (`unverified` is not enough); a failed one is refused with `attestation_failed`. **No phone build sends an attestation yet (the app has the hook, not the native module), so switching this on refuses every enrollment** until a build ships one. |
