...
**Error codes.** The full list is on the [Errors](/errors/) page. The ones that mean load or abuse: `rate_limited` (429, more than `RATE_LIMIT_REQUESTS_PER_IP` challenge or discovery calls from one IP per minute, default 60), `client_rate_limited` (429, one site started more than `RATE_LIMIT_CHALLENGES_PER_CLIENT` logins per minute, default 300), `push_rate_limited` (429, more than 10 pushes to one device per minute). Per-IP limits and the pairing `last_ip` read `CF-Connecting-IP`; `X-Forwarded-For` is believed only with `TRUST_PROXY_HEADERS=true`, which you set when a proxy you control (Caddy, nginx, a load balancer) sits in front of the container, because anyone can send that header. Without a trusted address nothing is limited by IP. Signed request bodies are capped at 64 KiB and any declared body over 1 MiB is refused (`payload_too_large`, 413). `BLE_LOOKUP_LIMIT` (default 5000) bounds how many recently seen phones a Bluetooth discovery scans. `CORS_ALLOWED_ORIGINS` lists the web origins, besides `APP_URL`, that may call `/me/*` from a browser; the login and token endpoints answer any origin, as OIDC public clients need. Webhooks and logout tokens a site did not take are retried by the Worker's cron trigger (`triggers.crons`, every five minutes, in `wrangler.jsonc`); the container fires the same job itself every `SCHEDULED_INTERVAL` seconds (default 300, `0` to disable on all but one replica), so nothing is lost while a site is down for up to two days. The same job applies retention: ended sessions, resolved verifications and settled deliveries after thirty days, unverified registrations after two, audit events after `AUDIT_RETENTION_DAYS` (default 365; `0` keeps them).
