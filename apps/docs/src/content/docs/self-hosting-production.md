...
| Configuration   | `INDEX_URL`, `APP_URL`, `PUSH_PROVIDER`, `OPEN_SITE_REGISTRATION`, `DASHBOARD_CLIENT_IDS`, rate limits. `OIDC_PKCE_OPTIONAL` must be unset                                                                                                                         | Environment variables in `wrangler.jsonc` `vars` or on the `index` container. Keep them in version control.                                                           |
